...
<Warning>
  **Use different values** for `JWT_SECRET` and `JWT_REFRESH_SECRET`. Both must be at least
  32 characters. Never commit real secrets to version control — `.env` is already in
  `.gitignore`.
</Warning>

<Accordion title="What happens if I skip this?">
  The placeholder secrets in `.env.example` work fine for local development. But if you
  deploy with the default values, anyone who reads the source code can forge authentication
  tokens for your API. Always generate unique secrets before deploying.
</Accordion>
